...
lost_sound = pygame.mixer.Sound(load_sound('sfx_die.wav'))
port = "COM10"
baudrate = 250000
...
def draw_window(surface):
    ...
    surface.blit(label, (top_left_x + play_width / 2 - (label.get_width() / 2) - 300, 200))
    for i in range(len(grid)):
        for j in range(len(grid[i])):
...
def main_menu():
    ...
    while run:
        ...
        if ser.in_waiting > 0:
            data = ser.read().decode('utf-8')
            if data == 'u' or data == 'd' or data == 'r' or data == 'l':    
                main()
